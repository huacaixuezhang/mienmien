...
if ! command -v /usr/libexec/java_home >/dev/null 2>&1; then
  echo "当前系统不支持 /usr/libexec/java_home，无法自动校验 JDK 21。"
  echo "请手动设置 JAVA_HOME 后执行：bash scripts/dev-check.sh"
  exit 1
fi
...
if [[ -z "${JAVA21_HOME}" ]]; then
  ...
  echo "请先安装 JDK 21，再执行：bash scripts/dev-check-jdk21.sh"
  exit 1
fi

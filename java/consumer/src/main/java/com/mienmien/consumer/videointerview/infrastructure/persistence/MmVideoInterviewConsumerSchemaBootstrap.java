...
/**
 * consumer 独立启动时确保视频面试表存在（与 business 侧迁移脚本等价）。
 */
@Component
@Order(0)
public class MmVideoInterviewConsumerSchemaBootstrap implements ApplicationRunner {

    private static final Logger log = LoggerFactory.getLogger(MmVideoInterviewConsumerSchemaBootstrap.class);

    private final JdbcTemplate jdbcTemplate;
    private final boolean enabled;

    public MmVideoInterviewConsumerSchemaBootstrap(
            JdbcTemplate jdbcTemplate,
            @Value("${mienmien.consumer.schema.auto-patch-video-interview:true}") boolean enabled) {
        ...
        try {
            ...
            jdbcTemplate.execute(
                    """
                            ...
                            """);
            ensureTurnBridgingColumn();
            log.debug("consumer 已确保 mm_video_interview_session / mm_video_interview_event / mm_video_interview_turn 存在");
        } catch (Exception e) {
            log.error("consumer 视频面试表自检失败，请执行 scripts/migrate-mm-video-interview-session.sql", e);
            throw new IllegalStateException("mm_video_interview_session 表创建失败: " + e.getMessage(), e);
        }
    }

    private void ensureTurnBridgingColumn() {
        try {
            Integer cnt =
                    jdbcTemplate.queryForObject(
                            """
                                    ...
                                    WHERE TABLE_SCHEMA = DATABASE()
                                      AND TABLE_NAME = 'mm_video_interview_turn'
                                      AND COLUMN_NAME = 'bridging_utterance'
                                    """,
                            Integer.class);
            if (cnt != null && cnt == 0) {
                jdbcTemplate.execute(
                        "ALTER TABLE mm_video_interview_turn ADD COLUMN bridging_utterance VARCHAR(2000) NULL AFTER agent_raw_json");
                log.info("consumer 已为 mm_video_interview_turn 增加 bridging_utterance 列");
            }
        } catch (Exception e) {
            log.warn("consumer 检查/追加 bridging_utterance 列失败（可手动执行 scripts/migrate-mm-video-interview-turn-bridging.sql）", e);
        }
    }
}

...
public class JdbcSpaceRepository implements SpaceRepository {
    ...
    private static final RowMapper<Space> MAPPER = (rs, rowNum) -> Space.restore(
    ...
    public void save(Space space) {
        jdbcTemplate.update(
                """
                        INSERT INTO mm_space (space_id, owner_user_id, name, status, created_at, updated_at, deleted_at)
                        VALUES (?,?,?,?,?,?,?)
                        """,
                space.getSpaceId(),
    ...
    public void update(Space space) {
        jdbcTemplate.update(
                """
                        UPDATE mm_space SET name=?, status=?, updated_at=?, deleted_at=?
                        WHERE space_id=?
                        """,
                space.getName(),
                space.getStatus(),
    ...
    public List<Space> findAllActiveOrderByCreatedAtDesc() {
        return jdbcTemplate.query(
                """
                        SELECT space_id, owner_user_id, name, status, created_at, updated_at, deleted_at
                        FROM mm_space WHERE status = 'ACTIVE' AND deleted_at IS NULL ORDER BY created_at DESC
                        """,
                MAPPER
        );
    }
    ...
    public List<Space> findAllRecycledOrderByDeletedAtDesc() {
        return jdbcTemplate.query(
                """
                        SELECT space_id, owner_user_id, name, status, created_at, updated_at, deleted_at
                        FROM mm_space WHERE status = 'RECYCLED' ORDER BY deleted_at DESC
                        """,
                MAPPER
        );
    }
    ...
    public List<Space> findActiveByOwnerUserIdOrderByCreatedAtDesc(String ownerUserId) {
        return jdbcTemplate.query(
                """
                        SELECT space_id, owner_user_id, name, status, created_at, updated_at, deleted_at
                        FROM mm_space WHERE owner_user_id = ? AND status = 'ACTIVE' AND deleted_at IS NULL
    ...
    public List<Space> findRecycledByOwnerUserIdOrderByDeletedAtDesc(String ownerUserId) {
        return jdbcTemplate.query(
                """
                        SELECT space_id, owner_user_id, name, status, created_at, updated_at, deleted_at
                        FROM mm_space WHERE owner_user_id = ? AND status = 'RECYCLED' ORDER BY deleted_at DESC
                        """,
                MAPPER,
                ownerUserId
        );
